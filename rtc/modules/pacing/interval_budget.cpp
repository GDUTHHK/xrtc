...
{
    set_target_bitrate_kbps(target_bitrate_kbps_);
}

IntervalBudget::~IntervalBudget() {
}

void IntervalBudget::set_target_bitrate_kbps(int target_bitrate_kbps) {
    target_bitrate_kbps_ = target_bitrate_kbps;
    //可能会因为某些原因导致处理packet的时间会很长，可能导致流逝时间会比较大，所以设置一个最大限制值
